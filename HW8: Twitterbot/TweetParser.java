...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        if ((csvColumn > slice.length - 1) || (csvColumn < 0)) {
            return null;
        }
        return slice[csvColumn];
    }
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        FileLineIterator files = new FileLineIterator(br);
        if (tweetColumn < 0) {
            return tweets;
        }
        while (files.hasNext()) {
            String csvLine = files.next();
            String tweet = extractColumn(csvLine, tweetColumn);
            if (tweet != null) {
                tweets.add(tweet);
            }
        }
    ...
    static List<String> parseAndCleanSentence(String sentence) {
        List<String> cleanWords = new LinkedList<>();
        String[] words = sentence.split(" ");
        for (String word : words) {
            if (cleanWord(word) != null) {
                cleanWords.add(cleanWord(word));
            }
        }
        return cleanWords;
    }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        String t = removeURLs(tweet);
        List<String> s = tweetSplit(t);
        for (String sentence : s) {
            words.add(parseAndCleanSentence(sentence));
        }
        return words;
    }
     ...
     */
    public static List<List<String>> csvDataToTrainingData(BufferedReader br, int tweetColumn) {
        List<String> tweet = csvDataToTweets(br, tweetColumn);
        List<List<String>> training = new LinkedList<>();
        if (tweetColumn < 0) {
            return training;
        }
        for (String s : tweet) {
            training.addAll(parseAndCleanTweet(s));
        }
        return training;
    }
}