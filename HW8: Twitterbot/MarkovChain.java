...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        if (first == null) {
            throw new IllegalArgumentException("parameter first is null");
        }
        if (second == null) {
            throw new IllegalArgumentException("parameter second is null");
        }
        if (chain.containsKey(first)) {
            ProbabilityDistribution<String> existingEntry = chain.get(first);
            existingEntry.record(second);
            chain.put(first, existingEntry);
        } else {
            ProbabilityDistribution<String> newEntry = new ProbabilityDistribution<>();
            newEntry.record(second);
            chain.put(first, newEntry);
        }
    }
    ...
    public void train(Iterator<String> sentence) {
        if (sentence == null) {
            throw new IllegalArgumentException("sentence Iterator is null");
        }
        String first = "";
        String second = first;
    ...
    public void reset(String start) {
        ...
        try {
            ...
            throw new NoSuchElementException("value not found in dataset");
        }
    }
    ...
    public String next() {
        String random = nw;
        if (!hasNext()) {
            throw new NoSuchElementException("No more words on the walk");
        }
        if (get(nw) != null) {
            nw = get(nw).pick(ng);
        } else {
