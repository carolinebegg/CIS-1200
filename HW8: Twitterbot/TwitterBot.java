...
public class TwitterBot {
    ...
    public TwitterBot(BufferedReader br, int tweetColumn, NumberGenerator ng) {
        ...
        this.ng = ng;
        if (tweetColumn < 0) {
            throw new IllegalArgumentException("tweetColumns are indexed at zero");
        }
        List<List<String>> training = TweetParser.csvDataToTrainingData(br, tweetColumn);
        for (List<String> t : training) {
            if ((t != null) && (!t.isEmpty())) {
                mc.train(t.iterator());
            }
        }
    ...
    public String generateTweet(int numWords) {
        ...
        if (numWords < 0) {
            throw new IllegalArgumentException("Error: numWords must be greater than zero.");
        } else if (numWords == 0) {
            return tweet.toString();
        } else {
            if (!mc.chain.isEmpty()) {
                tweet.append(mc.next());
                int wordCount = 1;
                while (wordCount <= numWords) {
                    if (wordCount == numWords) {
                        tweet.append(randomPunctuation());
                        return tweet.toString();
                    } else if (!mc.hasNext()) {
                        tweet.append(randomPunctuation());
                        mc.reset();
                    } else {
                        tweet.append(" ").append(mc.next());
                        wordCount++;
                    }
                }
            }
