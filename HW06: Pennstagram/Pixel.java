...
public class Pixel implements Comparable<Pixel> {
    ...
    private int[] lengthFixer(int[] len) {
        if (len == null) {
            len = new int[] { 0, 0, 0 };
        } else if (len.length < 3) {
            int[] copyArr = { 0, 0, 0 };
            System.arraycopy(len, 0, copyArr, 0, len.length);
            len = copyArr;
        }
        return len;
    }
    ...
    public int[] getComponents() {
        int[] newArr = { 0, 0, 0 };
        System.arraycopy(color, 0, newArr, 0, 3);
        return newArr;
    }
    ...
    public int distance(Pixel px) {
        ...
        if (px == null) {
            ...
            for (int i = 0; i < 3; i++) {
                distance += Math.abs(color[i] - px.color[i]);
            }
        }
        return distance;
    }
