...
public class SimpleManipulations {
    ...
    /*** Round (double) and Cast (to int) ***/
    public static int rndCst(double input) {
        return (int) Math.round(input);
    }

    /*** int value of red in a given pixel p ***/
    public static int red(Pixel p) {
        return p.getRed();
    }

    /*** int value of green in a given pixel p ***/
    public static int green(Pixel p) {
        return p.getGreen();
    }

    /*** int value of blue in a given pixel p ***/
    public static int blue(Pixel p) {
        return p.getBlue();
    }
    ...
    public static PixelPicture border(PixelPicture pic, int borderWidth, Pixel borderColor) {
        ...
        int border = 2 * borderWidth;

        Pixel[][] src = pic.getBitmap();
        Pixel[][] tgt = new Pixel[h + border][w + border];

        for (int row = 0; row < h + border; row++) {
            for (int col = 0; col < w + border; col++) {
                if (row < borderWidth || row >= h + borderWidth) {
                    tgt[row][col] = borderColor;
                } else if (col < borderWidth || col >= w + borderWidth) {
                    tgt[row][col] = borderColor;
                } else {
                    tgt[row][col] = src[row - borderWidth][col - borderWidth];
                }
            }
        }
        return new PixelPicture(tgt);
    ...
    public static PixelPicture invertColors(PixelPicture pic) {
        ...
        Pixel[][] inv = pic.getBitmap();
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = inv[row][col];
                int r = Math.abs(255 - red(p));
                int g = Math.abs(255 - green(p));
                int b = Math.abs(255 - blue(p));
                inv[row][col] = new Pixel(r, g, b);
            }
        }
        return new PixelPicture(inv);
    }
    ...
    public static PixelPicture grayScaleAverage(PixelPicture pic) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                int g = green(p);
                int b = blue(p);
                int gsAvg = rndCst((r + g + b) / 3.0);
                gsa[row][col] = new Pixel(gsAvg, gsAvg, gsAvg);
            }
        }
        return new PixelPicture(gsa);
    }
    ...
    public static PixelPicture scaleColors(
        ...
        Pixel[][] scl = pic.getBitmap();
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = scl[row][col];
                int r = rndCst(rfactor * red(p));
                int g = rndCst(gfactor * green(p));
                int b = rndCst(bfactor * blue(p));
                scl[row][col] = new Pixel(r, g, b);
            }
        }
        return new PixelPicture(scl);
    }
    ...
    public static int weightedAverage(double alpha, int x, int y) {
        return rndCst(x * alpha + y * (1 - alpha));
    }
    ...
    public static PixelPicture alphaBlend(double alpha, PixelPicture pic, PixelPicture f) {
        ...
        Pixel[][] p1 = pic.getBitmap();
        Pixel[][] p2 = f.getBitmap();
        ...
        if ((w1 == w2) != (h1 == h2)) {
            ...
            for (int col = 0; col < w1; col++) {
                for (int row = 0; row < h1; row++) {
                    Pixel pix1 = p1[row][col];
                    Pixel pix2 = p2[row][col];
                    int r = weightedAverage(alpha, pix1.getRed(), pix2.getRed());
                    int g = weightedAverage(alpha, pix1.getGreen(), pix2.getGreen());
                    int b = weightedAverage(alpha, pix1.getBlue(), pix2.getBlue());
                    p1[row][col] = new Pixel(r, g, b);
                }
            }
        }
        return new PixelPicture(p1);
    }
